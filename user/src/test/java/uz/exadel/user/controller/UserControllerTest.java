...
class UserControllerTest {
    ...
    void getUserList() throws Exception {
        ...
        mockMvc.perform(get("/api/user")).andExpect(status().isOk()).andExpect(jsonPath("$.object", hasSize(1)));
    }
    ...
    void getUserById() throws Exception {
        ...
        mockMvc.perform(get("/api/user/{id}", save.getId())).andExpect(status().isOk()).andExpect(jsonPath("$.object.id", isA(String.class))).andExpect(jsonPath("$.object.firstName", is(user.getFirstName()))).andExpect(jsonPath("$.object.lastName", is(user.getLastName())));
    }

    @Test
    void updateUser() throws Exception {
        User save = userRepository.save(user);
        mockMvc.perform(put("/api/user/{id}", save.getId()).contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(userDto))).andExpect(status().isOk()).andExpect(jsonPath("$.object", is(save.getId())));
    }
